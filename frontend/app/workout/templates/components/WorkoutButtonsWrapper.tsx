...
const WorkoutButtonsWrapper = ({ from }: Props) => {
    ...
    if (from === "edit-template") {
        return (
            <WorkoutButtons onEndFunction={endTemplate} updatePopUpContent={saveChangesToWorkoutTemplatePopUpContent} cancelPopUpContent={discardChangesToWorkoutTemplatePopUpContent} deleteButtonPopUpContent={deleteWorkoutTemplatePopUpContent} onAddRedirectRoute={`/workout/templates/selectExercises?from=${from}`} />
        )
    }

    // add-template page has no "Delete Template" option
    return (
        <WorkoutButtons onEndFunction={endTemplate} postPopUpContent={saveWorkoutTemplatePopUpContent} cancelPopUpContent={discardWorkoutTemplatePopUpContent} onAddRedirectRoute={`/workout/templates/selectExercises?from=${from}`} />
    )
